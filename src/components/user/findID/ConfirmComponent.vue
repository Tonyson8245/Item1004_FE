<template>
  <div class="h-screen flex-grow sm:flex">
    <div class="grid place-items-center w-full sm:m-auto">
      <div class="hidden sm:block">
        <img class="w-36" src="@/assets/icon/logo_mobile.svg" alt="" />
      </div>
      <div
        class="text-left mt-20 sm:mt-5 w-full px-6 sm:text-center font-bold sm"
      >
        <div class="text-2xl sm:text-[30]">아이디를 잊으셨나요?</div>
        <div class="text-lg sm:text-[28]">걱정 마세요. 금방 찾아드릴게요!</div>
      </div>

      <div class="grid gap-2 place-items-center w-11/12 mt-8 sm:mt-16">
        <div class="w-full">
          <div class="p-1 pr-0 flex items-center">
            <input
              placeholder="휴대 전화번호 입력('-'제외)"
              class="flex-1 mr-2 rounded-lg border border-everly-mid_grey bg-white py-3 px-4 text-[#6B7280] outline-none focus:border-everly-dark focus:shadow-md text-xs sm:text-sm"
            />
            <button
              class="bg-everly-main text-white rounded-lg shadow-md p-3 text-xs sm:text-sm"
            >
              인증번호 발송
            </button>
          </div>
          <div class="p-1 pr-0 flex items-center">
            <input
              placeholder="인증 번호 6자리를 입력하세요"
              class="flex-1 mr-2 rounded-lg border border-everly-mid_grey bg-white py-3 px-4 text-[#6B7280] outline-none focus:border-everly-dark focus:shadow-md text-xs sm:text-sm"
            />
            <button
              class="bg-everly-main text-white rounded-lg shadow-md p-3 text-xs sm:text-sm"
            >
              인증번호 확인
            </button>
          </div>
        </div>
      </div>

      <div class="grid p-1 gap-2 place-items-center w-full mt-4 sm:mt-8">
        <button
          class="h-11 lg:h-15 px-6 py-2 rounded-lg text-sm text-everly-white bg-everly-main w-11/12"
        >
          아이디 찾기
        </button>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts"></script>

<style scoped></style>
