<template>
  <div
    :class="{
      ' bg-op-40-right-blue ': route.params.channelId === props.channel.id,
    }"
  >
    <div
      @click="selectChannel"
      class="border-b-2 hover:opacity-40 cursor-pointer border-everly-mid_grey mx-2 py-5"
    >
      <div :class="{ 'items-center': lastMessageText() }" class="flex mx-3">
        ...
        <div class="flex flex-col w-full">
          <div class="flex w-full items-baseline">
            ...
            <p class="text-xs ml-auto text-everly-dark_grey">
              {{ lastMessageTime() }}
            </p>
          </div>
          ...
          <div class="flex">
            ...
            <div
              v-if="
                ...
                route.params.channelId !== props.channel.id
              "
              class="ml-auto min-w-[1.25rem] w-auto h-5 rounded-full text-center bg-everly-red text-everly-white flex items-center justify-center"
            >
...
const lastMessageTime = () => {
  if (props.channel.lastMessage !== null) {
    ...
    return new Intl.DateTimeFormat("ko", { timeStyle: "short" }).format(
      new Date(props.channel.lastMessage.createdAt)
    );
...
const chatTargetUserName = () => {
  ...
  //@ts-ignore
  return props.channel.members.filter((x) => x.id !== client.value.userId)[0]
    .username;
};
