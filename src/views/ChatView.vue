...
import headerComponentVue from "../components/header/webHeader.vue";
...
// TalkPlus 초기화 실행
// const chatClient =
const { client, channels, selectedChannel, isNewChat, postItem } =
  ...
  try {
    ...
    client.value.on("event", async (data) => {
      ...
      if (data.type === "message") {
        ...
        //@ts-ignore
        if (data.message.userId !== client.value.userId)
...
async function setPostItem() {
  ...
  //@ts-ignore
    ...
    //@ts-ignore
    selectedChannel.value?.data.postIdx === undefined &&
...
async function setSelectedChannel() {
  ...
  if (!route.query.postId && route.params.channelId)
    await chatStore.getSelectedChannel(route.params.channelId);
}
